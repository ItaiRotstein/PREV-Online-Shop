            ...
            <div className={`${iseMenuShow ? 'h-[180px] py-2' : 'h-0'} transition-all duration-300 overflow-hidden font-normal`}>
                {colourList.map((colour, idx) =>
                    <div key={colour + idx} className="flex items-center py-1">
                        <input id={`colour-${colour}`} type="checkbox" value="" className="w-5 h-5 text-blue-600 bg-gray-100 rounded  dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600" />
                        <label htmlFor={`colour-${colour}`} className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                            {colour}
                        </label>
                    </div>
                )}
