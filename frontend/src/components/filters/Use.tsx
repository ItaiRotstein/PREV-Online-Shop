            ...
            <div className={`${iseMenuShow ? `${isViewMore ? "h-[320px] " : "h-[236px] "} py-2` : "h-0"} transition-all duration-300 overflow-hidden font-normal`}>
                {useList.map((use, idx) =>
                    <div key={use + idx} className="flex items-center py-1">
                        <input id={`use-${use}`} type="checkbox" value="" className="w-5 h-5 text-blue-600 bg-gray-100 rounded" />
                        <label htmlFor={`use-${use}`} className="ms-2 text-sm font-medium text-gray-900">
                            {use}
                        </label>
                    </div>
                )}
