            ...
            <div className={`${iseMenuShow ? `${isViewMore ? 'h-[540px] ' : 'h-[236px] '} py-2` : 'h-0'} transition-all duration-300 overflow-hidden font-normal`}>
                {categoryList.map((category, idx) =>
                    <div key={category + idx} className="flex items-center py-1">
                        <input id={`category-${category}`} type="checkbox" value="" className="w-5 h-5 text-blue-600 bg-gray-100 rounded  dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600" />
                        <label htmlFor={`category-${category}`} className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                            {category}
                        </label>
                    </div>
                )}
